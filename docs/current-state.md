...
- `seed:j1:teams` / `verify:j1:teams` の実行
...
- J1 full team master data を確定する
- チーム名、Firestore doc id、API-SPORTS team ID、logo URL、aliases を確認する
- UI 表示は `Jリーグ`、内部 `competitionKey` は `football_j1` を維持する

### Task 2: confirmed J1 teams を `j1Teams.js` に追加
...
### Task 3: `seed:j1:teams` / `verify:j1:teams` 実行
- confirmed team master 追加後に `seed:j1:teams` を実行する
- 続けて `verify:j1:teams` で document field と query を確認する
...
### Task 5: sample games / model gaps を検討
