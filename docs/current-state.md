...
- `j1TeamsTodo` は空
- `seed:j1:teams` / `verify:j1:teams` は 1件時点では PASS
- 20クラブ追加後の Firestore seed は未実行
...
### Task 1: actual `seed:j1:teams` / `verify:j1:teams` 実行
- current J1 20クラブの generic dry-run は PASS
- actual 実行は承認後に `seed:j1:teams` / `verify:j1:teams` または generic non-dry で行う
- Firestore に current J1 20クラブが投入されることを確認する
- `j1TeamsTodo` が空のまま維持されることを確認する

### Task 2: Flutter UI で複数チーム検索・フォロー・ホーム表示確認
