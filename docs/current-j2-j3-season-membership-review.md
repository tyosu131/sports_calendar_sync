...
Additional evidence correction note:

- Batch 5 additional evidence correction documented: yes
- corrected candidates:
  ...
  - `hokkaido_consadole_sapporo`
- reason:
  - align Batch 5 candidate list and approval decision reviews with existing API-SPORTS Verification Tracker
- actual module entries added: 0
- `teamIdStatuses` changed: 0
- `seedable: true` changes: 0
- Firestore writes: 0
...
## Per-Club Approval Decision Review - sc_sagamihara

Summary:

- Approval decision review documented: yes
- Reviewed club: `sc_sagamihara`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
- deploy: 0
- Seedable rows changed: 0
- Batch 5 table changed: no

| check | review note | result |
|---|---|---|
| candidate internal team id | `sc_sagamihara` / `ＳＣ相模原` | pass |
| API raw team name | `Sagamihara` from `J3 2024 / 100` lookup source | pass |
| externalTeamId | `4324` | pass |
| logoUrl | `https://media.api-sports.io/football/teams/4324.png` | pass |
| name variance review | API raw name omits `SC`; keep as reviewed variance before module entry | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision identified for this candidate | pass |
| season membership separation | API evidence supports stable club identity only, not season seed data | pass |
| duplicate `/teams/{id}` policy | Use one stable `/teams/sc_sagamihara` document if later approved; do not duplicate by division or season | pass |
| target module candidate | `j3Teams.js` | pass |

Decision note:

- `sc_sagamihara` may move forward as an `approved-for-module-entry-candidate` in documentation.
- This is not an actual `approved-for-module-entry` file change.
- Do not add `sc_sagamihara` to `j2Teams.js` or `j3Teams.js` from this review alone.
- Keep `seedable` as `no` until a separate confirmed module entry approval and later seed approval are completed.

## Per-Club Approval Decision Review - thespa_gunma

Summary:

- Approval decision review documented: yes
- Reviewed club: `thespa_gunma`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
| check | review note | result |
|---|---|---|
| candidate internal team id | `thespa_gunma` / `ザスパ群馬` | pass |
| API raw team name | `Thespakusatsu Gunma` from `J2 2024 / 99` lookup source | pass |
| externalTeamId | `756` | pass |
| logoUrl | `https://media.api-sports.io/football/teams/756.png` | pass |
| name variance review | API raw name uses older / longer form; keep as reviewed variance before module entry | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision identified for this candidate | pass |
| season membership separation | API evidence supports stable club identity only, not season seed data | pass |
| duplicate `/teams/{id}` policy | Use one stable `/teams/thespa_gunma` document if later approved; do not duplicate by division or season | pass |
| target module candidate | `j2Teams.js` | pass |

Decision note:

- `thespa_gunma` may move forward as an `approved-for-module-entry-candidate` in documentation.
- This is not an actual `approved-for-module-entry` file change.
- Do not add `thespa_gunma` to `j2Teams.js` or `j3Teams.js` from this review alone.
- Keep `seedable` as `no` until a separate confirmed module entry approval and later seed approval are completed.

## Per-Club Approval Decision Review - iwaki_fc

Summary:

- Approval decision review documented: yes
- Reviewed club: `iwaki_fc`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
| check | review note | result |
|---|---|---|
| candidate internal team id | `iwaki_fc` / `いわきＦＣ` | pass |
| API raw team name | `Iwaki` from `J2 2024 / 99` lookup source | pass |
| externalTeamId | `7127` | pass |
| logoUrl | `https://media.api-sports.io/football/teams/7127.png` | pass |
| name variance review | API raw name omits `FC`; keep as reviewed variance before module entry | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision identified for this candidate | pass |
| season membership separation | API evidence supports stable club identity only, not season seed data | pass |
| duplicate `/teams/{id}` policy | Use one stable `/teams/iwaki_fc` document if later approved; do not duplicate by division or season | pass |
| target module candidate | `j2Teams.js` | pass |

Decision note:

- `iwaki_fc` may move forward as an `approved-for-module-entry-candidate` in documentation.
- This is not an actual `approved-for-module-entry` file change.
- Do not add `iwaki_fc` to `j2Teams.js` or `j3Teams.js` from this review alone.
- Keep `seedable` as `no` until a separate confirmed module entry approval and later seed approval are completed.

## Per-Club Approval Decision Review - rb_omiya_ardija

Summary:

- Approval decision review documented: yes
- Reviewed club: `rb_omiya_ardija`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
| check | review note | result |
|---|---|---|
| candidate internal team id | `rb_omiya_ardija` / `ＲＢ大宮アルディージャ` | pass |
| API raw team name | `Omiya Ardija` from `J3 2024 / 100` lookup source | pass |
| externalTeamId | `313` | pass |
| logoUrl | `https://media.api-sports.io/football/teams/313.png` | pass |
| name variance review | API raw name omits current `RB` branding; keep as reviewed variance before module entry | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision identified for this candidate | pass |
| season membership separation | API evidence supports stable club identity only, not season seed data | pass |
| duplicate `/teams/{id}` policy | Use one stable `/teams/rb_omiya_ardija` document if later approved; do not duplicate by division or season | pass |
| target module candidate | `j2Teams.js` | pass |

Decision note:

- `rb_omiya_ardija` may move forward as an `approved-for-module-entry-candidate` in documentation.
- This is not an actual `approved-for-module-entry` file change.
- Do not add `rb_omiya_ardija` to `j2Teams.js` or `j3Teams.js` from this review alone.
- Keep `seedable` as `no` until a separate confirmed module entry approval and later seed approval are completed.
- The `RB` branding variance must remain documented in the later exact diff plan and actual module entry source note.

## Per-Club Approval Decision Review - hokkaido_consadole_sapporo

Summary:

- Approval decision review documented: yes
- Reviewed club: `hokkaido_consadole_sapporo`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
| check | review note | result |
|---|---|---|
| candidate internal team id | `hokkaido_consadole_sapporo` / `北海道コンサドーレ札幌` | pass |
| API raw team name | `Consadole Sapporo` from `J1 2024 / 98` lookup source | pass |
| externalTeamId | `279` | pass |
| logoUrl | `https://media.api-sports.io/football/teams/279.png` | pass |
| name variance review | API raw name omits `Hokkaido`; keep as reviewed variance before module entry | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision identified for this candidate | pass |
| season membership separation | `J1 2024 / 98` lookup source is stable identity evidence only and does not imply permanent division membership | pass |
| duplicate `/teams/{id}` policy | Use one stable `/teams/hokkaido_consadole_sapporo` document if later approved; do not duplicate by division or season | pass |
| target module candidate | `j2Teams.js` | pass |

Decision note:

- `hokkaido_consadole_sapporo` may move forward as an `approved-for-module-entry-candidate` in documentation.
- This is not an actual `approved-for-module-entry` file change.
- Do not add `hokkaido_consadole_sapporo` to `j2Teams.js` or `j3Teams.js` from this review alone.
- Keep `seedable` as `no` until a separate confirmed module entry approval and later seed approval are completed.
- The `J1 2024 / 98` lookup source is stable identity evidence only and does not imply permanent division membership.

## Batch 5 Completion Note

- Batch 5 individual reviews completed: 5 / 5
- Batch 5 approved-for-module-entry-candidate rows: 5
- approved rows:
  - `sc_sagamihara`
  - `thespa_gunma`
  - `iwaki_fc`
...
## Batch 7 teamIdStatuses Exact Diff Plan

- Batch 7 `teamIdStatuses` exact diff plan documented: yes
- target data module:
  - `functions/scripts/data/competitionSeasonMemberships.js`
- target competitionSeasonKey:
  - `football_j2_j3_2026_hyakunen`
...
| teamId | current status | planned status | reason | implementation status |
|---|---|---|---|---|
| `tegevajaro_miyazaki` | `candidate_not_confirmed` | `confirmed_team_master` | Batch 7 actual `j3Teams.js` confirmed entry exists | `planned-not-written` |
| `sagan_tosu` | `candidate_not_confirmed` | `confirmed_team_master` | Batch 7 actual `j2Teams.js` confirmed entry exists | `planned-not-written` |
| `kagoshima_united` | `candidate_not_confirmed` | `confirmed_team_master` | Batch 7 actual `j2Teams.js` confirmed entry exists | `planned-not-written` |
| `renofa_yamaguchi` | `candidate_not_confirmed` | `confirmed_team_master` | Batch 7 actual `j2Teams.js` confirmed entry exists | `planned-not-written` |
| `roasso_kumamoto` | `candidate_not_confirmed` | `confirmed_team_master` | Batch 7 actual `j2Teams.js` confirmed entry exists | `planned-not-written` |
...
## Per-Club Approval Decision Review - oita_trinita

Summary:

- Approval decision review documented: yes
- Reviewed club: `oita_trinita`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
- deploy: 0
- Seedable rows changed: 0
- Batch 8 table changed: no
- season membership separation: pass
- duplicate `/teams/{id}` policy: pass
- this is not an actual file change
- do not add actual module entries from this review alone

Review detail:

- club nameJa: `大分トリニータ`
- API raw team name: `Oita Trinita`
- externalTeamId: `298`
- logoUrl: `https://media.api-sports.io/football/teams/298.png`
- lookup source: `J2 2024 / 99`
- variance status: `direct-or-near-direct`
- target module candidate: `j2Teams.js`
- decision: `oita_trinita` advances docs-only to `approved-for-module-entry-candidate`.

| check | review note | result |
|---|---|---|
| candidate internal team id | `oita_trinita` is the reviewed stable ID candidate. | pass |
| API raw team name | `Oita Trinita` matches the candidate club identity. | pass |
| externalTeamId | `298` is recorded as API evidence. | pass |
| logoUrl | API logo URL for team `298` is recorded. | pass |
| name variance review | Direct / near-direct variance is acceptable for docs-only approval candidate status. | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision is documented for this candidate. | pass |
| season membership separation | Season membership remains separate from stable team master data. | pass |
| duplicate `/teams/{id}` policy | No duplicate `/teams/{id}` document is created by this review. | pass |
| target module candidate | Target module candidate is `j2Teams.js`. | pass |

## Per-Club Approval Decision Review - gainare_tottori

Summary:

- Approval decision review documented: yes
- Reviewed club: `gainare_tottori`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
- club nameJa: `ガイナーレ鳥取`
- API raw team name: `Gainare Tottori`
- externalTeamId: `4319`
- logoUrl: `https://media.api-sports.io/football/teams/4319.png`
- lookup source: `J3 2024 / 100`
- variance status: `direct-or-near-direct`
- target module candidate: `j3Teams.js`
- decision: `gainare_tottori` advances docs-only to `approved-for-module-entry-candidate`.

| check | review note | result |
|---|---|---|
| candidate internal team id | `gainare_tottori` is the reviewed stable ID candidate. | pass |
| API raw team name | `Gainare Tottori` matches the candidate club identity. | pass |
| externalTeamId | `4319` is recorded as API evidence. | pass |
| logoUrl | API logo URL for team `4319` is recorded. | pass |
| name variance review | Direct / near-direct variance is acceptable for docs-only approval candidate status. | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision is documented for this candidate. | pass |
| season membership separation | Season membership remains separate from stable team master data. | pass |
| duplicate `/teams/{id}` policy | No duplicate `/teams/{id}` document is created by this review. | pass |
| target module candidate | Target module candidate is `j3Teams.js`. | pass |

## Per-Club Approval Decision Review - giravanz_kitakyushu

Summary:

- Approval decision review documented: yes
- Reviewed club: `giravanz_kitakyushu`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
- club nameJa: `ギラヴァンツ北九州`
- API raw team name: `Kitakyushu`
- externalTeamId: `805`
- logoUrl: `https://media.api-sports.io/football/teams/805.png`
- lookup source: `J3 2024 / 100`
- variance status: `name-variance-reviewed`
- variance note:
  - API raw name is shortened to `Kitakyushu`.
  - This variance has already been tracked as a name variance row.
  - Treat it as acceptable stable identity evidence for docs-only module entry candidate approval.
- target module candidate: `j3Teams.js`
- decision: `giravanz_kitakyushu` advances docs-only to `approved-for-module-entry-candidate`.

| check | review note | result |
|---|---|---|
| candidate internal team id | `giravanz_kitakyushu` is the reviewed stable ID candidate. | pass |
| API raw team name | `Kitakyushu` is a shortened API raw name already handled as name variance evidence. | pass |
| externalTeamId | `805` is recorded as API evidence. | pass |
| logoUrl | API logo URL for team `805` is recorded. | pass |
| name variance review | Shortened API raw name is acceptable for docs-only approval candidate status. | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision is documented for this candidate. | pass |
| season membership separation | Season membership remains separate from stable team master data. | pass |
| duplicate `/teams/{id}` policy | No duplicate `/teams/{id}` document is created by this review. | pass |
| target module candidate | Target module candidate is `j3Teams.js`. | pass |

## Per-Club Approval Decision Review - fc_ryukyu

Summary:

- Approval decision review documented: yes
- Reviewed club: `fc_ryukyu`
- Review result: `approved-for-module-entry-candidate`
- Actual module entry added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
...
- club nameJa: `ＦＣ琉球`
- API raw team name: `FC Ryukyu`
- externalTeamId: `2235`
- logoUrl: `https://media.api-sports.io/football/teams/2235.png`
- lookup source: `J3 2024 / 100`
- variance status: `direct-or-near-direct`
- target module candidate: `j3Teams.js`
- decision: `fc_ryukyu` advances docs-only to `approved-for-module-entry-candidate`.

| check | review note | result |
|---|---|---|
| candidate internal team id | `fc_ryukyu` is the reviewed stable ID candidate. | pass |
| API raw team name | `FC Ryukyu` matches the candidate club identity. | pass |
| externalTeamId | `2235` is recorded as API evidence. | pass |
| logoUrl | API logo URL for team `2235` is recorded. | pass |
| name variance review | Direct / near-direct variance is acceptable for docs-only approval candidate status. | pass |
| existing confirmed team ID collision | No existing confirmed team ID collision is documented for this candidate. | pass |
| season membership separation | Season membership remains separate from stable team master data. | pass |
| duplicate `/teams/{id}` policy | No duplicate `/teams/{id}` document is created by this review. | pass |
...
## Batch 8 Approval Decision Review Completion Note

- Batch 8 individual reviews completed: 4 / 4
- Batch 8 results:
  - `oita_trinita`: `approved-for-module-entry-candidate`
  - `gainare_tottori`: `approved-for-module-entry-candidate`
  - `giravanz_kitakyushu`: `approved-for-module-entry-candidate`
  - `fc_ryukyu`: `approved-for-module-entry-candidate`
- Actual module entries added: 0
- `j2Teams.js` entries added: 0
- `j3Teams.js` entries added: 0
- `teamIdStatuses` changed: 0
...
## Batch 8 teamIdStatuses Exact Diff Plan

- Batch 8 `teamIdStatuses` exact diff plan documented: yes
- target data module:
  - `functions/scripts/data/competitionSeasonMemberships.js`
- target competitionSeasonKey:
  - `football_j2_j3_2026_hyakunen`
...
|---|---|---|---|---|
| `oita_trinita` | `candidate_not_confirmed` | `confirmed_team_master` | `planned-not-written` | actual `j2Teams.js` confirmed entry exists |
| `gainare_tottori` | `candidate_not_confirmed` | `confirmed_team_master` | `planned-not-written` | actual `j3Teams.js` confirmed entry exists |
| `giravanz_kitakyushu` | `candidate_not_confirmed` | `confirmed_team_master` | `planned-not-written` | actual `j3Teams.js` confirmed entry exists |
| `fc_ryukyu` | `candidate_not_confirmed` | `confirmed_team_master` | `planned-not-written` | actual `j3Teams.js` confirmed entry exists |
...
## J2 / J3 2026 Firestore Read Verification Command Exact Plan

- Firestore read verification command exact plan documented: yes
- target competitionSeasonKey:
  - `football_j2_j3_2026_hyakunen`
- target Firestore document:
  - `competitionSeasonMemberships/football_j2_j3_2026_hyakunen`
- purpose:
  - verify the already-written Firestore season membership document by reading it back
- planned script:
...
- planned optional CLI:
  - `--season <competitionSeasonKey>`
- default behavior:
  - require explicit `--season`
  - do not verify all seasons by default
- Firestore write:
  - 0
- non-dry seed:
  - 0
...
- initialize Firebase Admin SDK only for Firestore read verification
- read `serviceAccountKey.json` only in this read verification command
- read exactly:
  - `competitionSeasonMemberships/{competitionSeasonKey}`
- do not write Firestore
- do not call `.set`, `.update`, `.add`, `.delete`, `.batch`, or `.commit`
- compare Firestore document against local `competitionSeasonMemberships.js` profile
- verify required fields:
  ...
  - `groups`
  - `teamIdStatuses`
- verify expected values for `football_j2_j3_2026_hyakunen`:
  - `status: 'seedable'`
  - `seedable: true`
  - groups: 4
  - total teamIds: 40
  - confirmed team references: 40
  - blocked/unconfirmed rows: 0
- verify Firestore `groups` match local profile:
  - group keys
  - display names
  - teamIds order, or explicitly document if order-insensitive comparison is used
- verify Firestore `teamIdStatuses` match local profile
- verify no missing local team IDs
- verify no extra Firestore team IDs
- verify `createdAt` exists
- verify `updatedAt` exists
- do not require exact timestamp value equality because server timestamps are generated by Firestore
...
Planned changed files for actual implementation:

- `functions/scripts/verifyCompetitionSeasonMembershipFirestore.js`
- optionally `docs/current-state.md` after implementation result
- do not modify `seedCompetitionSeasonMemberships.js` unless strictly necessary
- do not modify `competitionSeasonMemberships.js`
- do not modify `verifyCompetitionSeasonMemberships.js` unless strictly necessary

| target file | planned change | implementation status | reason |
|---|---|---|---|
| `functions/scripts/verifyCompetitionSeasonMembershipFirestore.js` | add read-only Firestore verification script | `planned-not-written` | verify actual written document |
| `docs/current-state.md` | record future implementation result after actual script is added and run | `planned-not-written` | keep project state traceable |

Decision:

- Exact plan result:
  - `ready-for-firestore-read-verification-script-approval`
- This is not the actual read verification implementation.
- This is not a Firestore read execution.
- This is not a Firestore write.
- Do not re-run `--write`.
- API sync and deploy remain separate tasks.
- npm audit vulnerabilities remain separate task.
